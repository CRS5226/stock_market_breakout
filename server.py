...
def monitor_stock(shared_data, initial_config):
    ...
    while True:
        try:
            ...
            send_error_alert(f"[{stock_code}] Monitor Error: {type(e).__name__}: {e}")
            time.sleep(5)
...
if __name__ == "__main__":
    try:
        ...
        print("📦 Stocks to monitor:", [s["stock_code"] for s in stock_list])
        ...
        for stock in stock_list:
            ...
            BREAKOUT_STATE[code] = {"above_resistance": False, "below_support": False}

            collector_proc = Process(target=start_collector, args=(shared_data, code))
            collector_proc.start()
            collector_processes.append(collector_proc)

            monitor_proc = Process(target=monitor_stock, args=(shared_data, stock))
            monitor_proc.start()
            monitor_processes.append(monitor_proc)

            send_pipeline_status("✅ Started Monitoring", code)

        for p in collector_processes + monitor_processes:
            p.join()
